...
//     const parsedOverviewCategoryIcons =
//       parseIfString(overviewCategoryIcons) || [];
//     const finalOverviewCategoryIcons = parsedOverviewCategoryIcons.map(
//       (cat, index) => ({
//         name: cat.name || cat,
...
export const getAllTravelPackages = async (req, res) => {
  try {
    const packages = await travelPackageModel.find().sort({ createdAt: -1 });
